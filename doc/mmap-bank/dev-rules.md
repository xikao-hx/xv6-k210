...
- SHARED 文件缓存键必须使用稳定文件身份和页对齐 offset，不能以 fd 或
  mmap object 地址代替文件身份。
- 缓存 entry 基础引用、mappings 和 PTE 物理引用必须成对维护；最后 mappings
  归零时必须在 dirty 写回完成后才能删除缓存键。
- 缓存自旋锁内只修改链表、状态和计数；页面/节点分配与 FAT32 I/O 必须在锁外。
- PRIVATE 文件页不得进入共享文件缓存。
