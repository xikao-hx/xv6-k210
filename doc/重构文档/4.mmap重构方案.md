...
**当前限制：** 
* 页对齐的 top − 页对齐的 length = 页对齐的 start。任意 length（非 PGSIZE 整数倍）经过 PGROUNDUP 后，start 依然是页对齐的。
* 进行拆分后会出现地址不对齐的问题出现，但是这个时候是通过vma中的地址范围，确定能访问的地址空间的范围

**关于分配都是一整页的分析：**
* 虚拟地址只能按页映射——PTE 粒度就是 PGSIZE，MMU 硬件决定，不可能按字节建立映射；
* 物理内存分配粒度也是页——kalloc_page 的 freelist 就是页链表；
* Linux 完全一样：mmap(NULL, 100, ...) + 访问 → 占一页虚拟地址 + 一页物理内存。这是所有现代 OS 的通用行为。
...
```text
匿名共享页 P，被进程 A、B 共享（fork），quota = 3（object 1 + A 1 + B 1）。

A 完整 munmap 自己的 vma（进入你贴的分支）：

vma_unmap_pages：解除 A 的 PTE → quota 3−1=2
mmap_object_put：object refcnt 2→1，不等于 0，不 destroy
→ P 安然无恙，B 还映射着、object 还持有锚引用。

B 随后退出：

vma_unmap_pages：quota 2−1=1
mmap_object_put：refcnt 1→0 → anon_object_destroy → 每页 kfree_page → quota 1−1=0 → 真正 free 一次
两次 kfree_page 分别归还了 PTE 那份和 object 那份引用，quota 单调递减到 0，恰好触发一次真实释放。这就是不重复释放的根本保证。
```
