...
**实现的关键点：**
* 从原 sleep() 抽出一个公共核心，多一个 interruptible 参数，共三处检查pending 信号：
  * 入睡前：如果进程已带着未决信号，干脆不睡，直接返回 -1 —— 信号优先；
  * 睡下时：置 p->interruptible_sleep = interruptible，作为「这次睡眠能被信号唤醒」的标记；
  * 醒来后：清除标记，若此时有 pending 信号则返回 -1 表示「被信号打断」。
* 发送信号置位 sig_pending 时，发现目标进程正处于可中断睡眠，就直接把它置成 RUNNABLE（相当于一次合成 wakeup）。这样调度器就会重新调度它，sched() 返回，sleep_on 醒来发现信号 → 返回 -1。
...
**实现的关键点：**
* 子进程继承父进程的进程组
* setgpid 把当前进程加入到某个进程组或者自己创建一个进程组
* console的 ioctl 支持把某个进程组设置为前台进程组
* uart 驱动中支持注册 observer ，observer 会检查 Ctrl + C 出现，并设置全局标志位
* 在 usertrap 函数中统一检查并发送信号给前台进程组的所有进程
...
**实现的关键点：**
* 区分前台组和前台组的所有者：shell是持有者、子进程是前台组，实现子进程退出后shell能变为前台组
* uart 中 rx_cancel_pending 是为了避免进程如果阻塞在 read() + TTY 模式 + 键盘注入 Ctrl+C，这个时候无法终止当前进程，因为信号的投递是在 userret 中完成的，而阻塞进程无法执行到这里，所以可中断睡眠无法发挥作用
* 当前如果 uart 处于 raw 模式，无法中断睡眠在 read 上的进程（burn.c）

sleep_interruptible 只能被已挂到进程 pending 位上的信号打断。问题在于：

| 特性 | 直接信号 (sigsend/kill) | 键盘 Ctrl+C |
|------|------------------------|-------------|
| **处理入口** | `signal.c:128 signal_mark_locked` | `console.c:35` 置 `tty_events` 标志 |
| **置 pending 时机** | 立即置 `sig_pending` | 延迟，等 `console_dispatch_events()` 在 trap 路径调用 `signal_send_pgrp` |
| **对 SLEEPING 进程** | 立即唤醒，`sleep_interruptible` 返回 -1 | 无 pending 位，进程继续睡眠，永不返回 |
