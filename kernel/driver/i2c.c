...
#define I2C_DMA_TIMEOUT_TICKS 100
...
void i2c_dw_init(i2c_device_number_t i2c_num) {
    ...
    i2c_adapter->dma_rdlr = 0;    // set up rx and tx burst size
...
static void i2c_dma_isr(void *data);
static void i2c_dw_isr(void *data);
void i2c_init(void) {
    ...
    for (int i = 0; i < I2C_DEVICE_MAX; i++) {
        ...
        struct i2c_dw_data *i2c_data = &i2c_ctrls[i]->i2c_data;
        i2c_dw_init(i);
        snprintf(names[i], sizeof(names[i]), "i2c_%d", i);
        initsleeplock(&i2c_ctrls[i]->lock, names[i]);
        initlock(&i2c_ctrls[i]->isr_lock, names[i]);
        irq_register(I2C0_IRQ + i, i2c_dw_isr, i2c_ctrls[i]);
        if (i2c_data->dma_enable) {
            if (i2c_data->chan_tx < DMAC_CHANNEL_MAX)
                irq_register(DMAC_CH0_IRQ + i2c_data->chan_tx, i2c_dma_isr,
                             (void *)(uintptr_t)i2c_data->chan_tx);
            if (i2c_data->chan_rx < DMAC_CHANNEL_MAX)
                irq_register(DMAC_CH0_IRQ + i2c_data->chan_rx, i2c_dma_isr,
                             (void *)(uintptr_t)i2c_data->chan_rx);
        }
    }
}

static void
i2c_dw_write(struct i2c_controller *i2c_ctrl)
{
  ...
  while (xfer->tx_len && (8 - i2c_adapter->txflr) > 0) {
    ...
    if (xfer->need_restart) {
        cmd |= I2C_DATA_CMD_RESTART;
        xfer->need_restart = 0;
    }

    if (xfer->is_lastmsg && xfer->tx_len == 1)
...
i2c_dw_read(struct i2c_controller *i2c_ctrl)
{
  ...
  while (xfer->rx_len && i2c_adapter->rxflr) {
    *xfer->rx_dst++ = (uint8)i2c_adapter->data_cmd;
    xfer->rx_len--;
  }
}
...
static void
i2c_dma_isr(void *data)
{
  dmac_intr((dmac_channel_number_t)(uintptr_t)data);
}

static void
i2c_dw_isr(void *data)
{
  struct i2c_controller *i2c_ctrl = data;
  volatile i2c_t *i2c_adapter = i2c[i2c_ctrl->bus_num];
  ...
  if (stat & I2C_INTR_STAT_TX_ABRT) {
    LOG_E("i2c_dw_isr: tx abort bus=%d abrt=%x\n", i2c_ctrl->bus_num,
...
// Send one message via the interrupt path.
static int
                  ...
                  int need_restart, int is_lastmsg)
{
  ...
  i2c_dw_write(i2c_ctrl);   /* prefill the first batch so the bus does not wait on the first TX_EMPTY */
                  ...
                  bool need_restart, bool is_lastmsg)
{
    ...
    ret = dmac_wait_idle_timeout(i2c_data->chan_tx, I2C_DMA_TIMEOUT_TICKS);
    acquire(&i2c_ctrl->isr_lock);
    if(ret < 0) {
...
// Receive one message via the interrupt path.
static int
                  ...
                  int need_restart, int is_lastmsg)
{
  ...
  i2c_dw_write(i2c_ctrl);   /* prefill the read commands */
  int ret = i2c_wait_xfer(i2c_ctrl, I2C_INT_TIMEOUT_TICKS);
  i2c_adapter->intr_mask = 0;
  release(&i2c_ctrl->isr_lock);
  return ret;
                  ...
                  bool need_restart, bool is_lastmsg)
{
    ...
    ret = dmac_wait_idle_timeout(i2c_data->chan_rx, I2C_DMA_TIMEOUT_TICKS);
    if(ret < 0)
...
int i2c_transfer(struct i2c_device *dev, struct i2c_msg *msgs, int num) {
    ...
    for (i = 0; i < num; i ++) {
        ...
        /* read/write: large frames go through DMA, small ones through the interrupt path */
