...
signal_deliver(struct proc *p)
{
  ...
  // Signal terminates process: SIGKILL or default-terminate signals
  if(signum == SIGKILL ||
  ...
  // Signal ignored: explicit ignore (SIG_IGN) or default-ignore (e.g., SIGCHLD)
  ...
  // emulation stackframe
  p->trapframe->a0 = signum;
  p->trapframe->ra = SIGTRAMP;   // all procs use same va
...
signal_sigreturn(struct proc *p)
{
  ...
  *p->trapframe = p->sig_saved_trapframe;
  ...
  // Return saved a0 to syscall() so it won't be overwritten
  // when the syscall framework sets trapframe->a0 = ret.
  return a0;  
}
