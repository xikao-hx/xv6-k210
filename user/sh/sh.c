...
main(void)
{
  ...
  while((command_status = getcmd(buf, sizeof(buf))) >= 0){
    ...
    // Parent: set child's process group as well (double insurance against race)
    // Even if child delays its setpgid, parent ensures the group is created
    ...
    // Parent: set self as foreground immediately
    if(!background &&
